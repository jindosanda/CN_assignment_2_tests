...
class TestClients(unittest.TestCase):
    ...
    def test8(self):
        ...
        self.assertEqual(output[0].lower(), 'get')
        self.assertEqual(output[1], '/')
        self.assertEqual(output[2].lower(), 'http/1.0')
        self.assertIn(output[3].lower(), ['host:10.40.0.46', 'host:10.40.0.46:9997'])
        self.assertEqual(output[4].lower(), 'http/1.0')
        self.assertEqual(output[5].lower(), '200')
        self.assertEqual(output[6].lower(), 'ok')
        self.assertEqual(output[7].lower(), 'server:')
        self.assertEqual(output[8].lower(), 'simplehttp/0.6')
        self.assertEqual(type(output[9]), str)
        self.assertEqual(output[10].lower(), 'date:')
        self.assertIn(output[11].lower()[:3], ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'])
        self.assertEqual(type(int(output[12])), int)
        self.assertIn(output[13].lower(), ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'])
        self.assertGreaterEqual(int(output[14]), 2021)
        self.assertEqual(len(output[15].lower()), 8)
        tmp = output[15].lower().split(':')
        self.assertEqual(type(int(tmp[0])), int)
        self.assertEqual(type(int(tmp[1])), int)
        self.assertEqual(type(int(tmp[2])), int)
        self.assertEqual(output[16].lower(), 'gmt')
        self.assertEqual(output[17].lower(), 'content-type:')
        self.assertEqual(output[18].lower(), 'text/html')
        self.assertEqual(output[19].lower(), 'content-length:')
        self.assertEqual(type(int(output[20])), int)
        self.assertEqual(output[21].lower(), 'last-modified:')
        self.assertIn(output[22].lower()[:3], ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'])
        self.assertEqual(type(int(output[23])), int)
        self.assertIn(output[24].lower(), ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'])
        self.assertGreaterEqual(int(output[25]), 2021)
        self.assertEqual(len(output[26].lower()), 8)
        tmp = output[26].lower().split(':')
        self.assertEqual(type(int(tmp[0])), int)
        self.assertEqual(type(int(tmp[1])), int)
        self.assertEqual(type(int(tmp[2])), int)
        self.assertEqual(output[27].lower(), 'gmt')
        self.assertEqual(output[28].lower(), "it's")
        self.assertEqual(output[29].lower(), 'not')
        self.assertEqual(output[30].lower(), 'that')
        self.assertEqual(output[31].lower(), 'easy')
        self.assertEqual(output[32].lower(), '<!--')
        self.assertTrue(check_flag_validity(8, output[33]))
        self.assertEqual(output[34].lower(), '-->')
